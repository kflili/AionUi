      ...
      const transformedSource = source.replace(
        ...
        function (str, match) {
          ...
          const importChunk = specifiers
            ...
            .join(', ');
          const hocChunk = specifiers
            .map(
              (s: { importName: string; localName: string }) => `const ${s.localName} = IconParkHOC(_${s.localName})`
            )
            .join(';\n          ');
          const importComponent = str.replace(match, importChunk);
